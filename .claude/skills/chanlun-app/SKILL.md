...
- **背驰结构确认 struct_ok**(2026-09):后段紧邻某已完成中枢(中枢最后一笔在
  i-2/i-1)且创出脱离中枢区间(ZD/ZG)的极值 = 标准背驰;否则=宽口径
  (单边行情中段的相邻笔动能衰减),评分×0.85 降档,图上「宽」前缀+降色,
  面板灰色「宽口径」标签。真实数据约 6-7 成背驰是宽口径。
- 趋势/盘整背驰:**以紧邻中枢为锚**——它与其前一个中枢同向依次移动=趋势背驰;
  无紧邻中枢(struct_ok=False)一律记盘整(修复:历史无关中枢不得参与分类,
  否则误得趋势 15 分。修复前约 3/4 的"趋势背驰"标签是错的)。
